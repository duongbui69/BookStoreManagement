...
namespace BookStoreManagement.Services
{
    public class PurchaseReceiptService : ServiceBase
    {
        ...
        public async Task<PurchaseReceipt?> GetByIdAsync(int id) { PermissionService.RequireAdmin(); Require(id > 0, "Id phiếu nhập không hợp lệ."); return await _repository.GetByIdAsync(id); }
        ...
        public async Task<List<PurchaseReceiptListViewModel>> GetByStoreIdAsync(int storeId) { PermissionService.RequireAdmin(); Require(storeId > 0, "Cửa hàng không hợp lệ."); return await _repository.GetByStoreIdAsync(storeId); }
        ...
        public async Task<List<PurchaseReceiptDetail>> GetDetailsAsync(int receiptId) { PermissionService.RequireAdmin(); Require(receiptId > 0, "Id phiếu nhập không hợp lệ."); return await _repository.GetDetailsAsync(receiptId); }
        ...
        public async Task<List<PurchaseReceiptListViewModel>> GetByDateRangeAsync(DateTime fromDate, DateTime toDate, int? storeId = null) { PermissionService.RequireAdmin(); Require(fromDate <= toDate, "Khoảng ngày không hợp lệ."); return await _repository.GetByDateRangeAsync(fromDate, toDate, storeId); }
        ...
        public void UpdateStatus(int id, string status) { PermissionService.RequireAdmin(); Require(id > 0, "Id phiếu nhập không hợp lệ."); Require(!string.IsNullOrWhiteSpace(status), "Trạng thái không hợp lệ."); _repository.UpdateStatus(id, status); }
        public async Task UpdateStatusAsync(int id, string status) { PermissionService.RequireAdmin(); Require(id > 0, "Id phiếu nhập không hợp lệ."); Require(!string.IsNullOrWhiteSpace(status), "Trạng thái không hợp lệ."); await _repository.UpdateStatusAsync(id, status); }
