...
{
    public class SupplierManagementForm : AdminFormBase
    {
        ...
        private void BtnUpdate_Click(object? s,EventArgs e){try{if(selectedId<=0){MessageBox.Show("Chọn NCC.");return;} _service.Update(Build()); MessageBox.Show("Cập nhật thành công."); LoadData();}catch(Exception ex){MessageBox.Show(ex.Message);}}
        private void BtnToggle_Click(object? s,EventArgs e){try{if(selectedId<=0){MessageBox.Show("Chọn NCC.");return;} _service.SetActive(selectedId,!chkActive.Checked); MessageBox.Show("Cập nhật trạng thái thành công."); LoadData();}catch(Exception ex){MessageBox.Show(ex.Message);}}
