...
{
    public class CategoryManagementForm : AdminFormBase
    {
        ...
        private void InitializeComponent()
        {
            ...
            txtName = CreateTextBox(top, "Tên danh mục", 20, 20);
        ...
        {
            try { dgv.DataSource = _service.GetAll(); }
        ...
        private void Dgv_CellClick(object? sender, DataGridViewCellEventArgs e)
        {
            ...
            txtName.Text = dgv.CurrentRow!.Cells["CategoryName"].Value?.ToString();
        ...
        private void BtnAdd_Click(object? sender, EventArgs e)
        {
            try
            {
                _service.Add(new Category { CategoryName = txtName.Text, Description = txtDescription.Text });
        ...
        private void BtnUpdate_Click(object? sender, EventArgs e)
        {
            try
            {
                if (selectedId <= 0) { MessageBox.Show("Chọn danh mục cần sửa."); return; }
                _service.Update(new Category { Id = selectedId, CategoryName = txtName.Text, Description = txtDescription.Text, IsActive = chkActive.Checked });
        ...
        private void BtnToggle_Click(object? sender, EventArgs e)
        {
            try
            {
                if (selectedId <= 0) { MessageBox.Show("Chọn danh mục."); return; }
                _service.SetActive(selectedId, !chkActive.Checked);
        ...
        {
            try { dgv.DataSource = _service.Search(txtSearch.Text); }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
        }
    }
}
