...
{
    public class PurchaseReceiptManagementForm : AdminFormBase
    {
        private readonly PurchaseReceiptService _service = new PurchaseReceiptService();
        ...
        public PurchaseReceiptManagementForm(){ Text="Quản lý phiếu nhập"; Width=1200; Height=720; InitializeComponent(); Load+=(s,e)=>LoadData(); }
        ...
        private void BtnDetails_Click(object? sender,EventArgs e){try{if(selectedId<=0){MessageBox.Show("Chọn phiếu nhập.");return;} dgvDetails.DataSource=_service.GetDetails(selectedId);}catch(Exception ex){MessageBox.Show(ex.Message);}}
        private void BtnExport_Click(object? sender,EventArgs e){try{using var sfd=new SaveFileDialog{Filter="Excel File|*.xlsx",FileName="PhieuNhap.xlsx"}; if(sfd.ShowDialog()==DialogResult.OK){_excelService.ExportDataGridView(dgv,sfd.FileName,"PhieuNhap"); MessageBox.Show("Xuất Excel thành công.");}}catch(Exception ex){MessageBox.Show(ex.Message);}}
    }
}
